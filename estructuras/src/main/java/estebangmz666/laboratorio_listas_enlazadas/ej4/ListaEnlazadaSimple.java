package estebangmz666.laboratorio_listas_enlazadas.ej4;
...
public class ListaEnlazadaSimple<T> implements Iterable<T> {
    ...
    public ListaEnlazadaSimple<Integer> obtenerNumerosImpares() {
        ListaEnlazadaSimple<Integer> listaImpares = new ListaEnlazadaSimple<>();
        Nodo<T> nodoActual = nodoPrimero;
    
        while (nodoActual != null) {
            Integer valor = (Integer) nodoActual.getDato();
            if (valor % 2 != 0) {
                listaImpares.agregarAlFinal(valor);
            }
            nodoActual = nodoActual.getSiguiente();
        }
